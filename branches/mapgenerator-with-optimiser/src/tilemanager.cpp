...
namespace mapgeneration
{
	...
	: _tile_cache(tile_cache), _service_list(service_list), _trace_queue(),
		_trace_queue_mutex(), _number_of_processed_traces(0),
		_number_of_processed_traces_mutex(), _invalid_path_score(false),
		_optimisation_mode(false),
		_single_trace_mode(false), _single_trace_mode_event()
	{
...
/*	std::vector< std::vector<Node> >
	...
	int
	TileManager::get_number_of_processed_traces()
	{
		ost::MutexLock lock(_number_of_processed_traces_mutex);
		
		return _number_of_processed_traces;
	}
	
	
	time_t
	TileManager::get_start_time() const
	{
		return _start_time;
	}
	...
	{
		#warning Where should we decide, if the trace is long enough. \
			Here, TraceFilter, ... Because you know, size does matter... :-)
		
		double search_radius_m;
		_service_list->get_service_value(
			"traceprocessor.search_radius_m", search_radius_m
		);
		filtered_trace.calculate_needed_tile_ids(search_radius_m * 2.5/*, false*/);
...
//		unlock_edge_tiles(edge_id);
	...
	TileManager::trace_processor_finished(unsigned int trace_processor_id)
	{
		...
		_number_of_processed_traces_mutex.enterMutex();
		++_number_of_processed_traces;
		_number_of_processed_traces_mutex.leaveMutex();
	}
	...
	TileManager::thread_run()
	{
		...
		while (!should_stop() || _trace_processors.size() > 0)
		{
			int new_trace_processors;
			if (_single_trace_mode)
				new_trace_processors = 1 - _trace_processors.size();
			else
				new_trace_processors = 1 - _trace_processors.size();
	...
	TileManager::delete_trace_processor(unsigned int trace_processor_id)
	{
		...
		std::vector< std::map<unsigned int, unsigned int>::iterator >::iterator
			iter2 = iterators.begin();
		std::vector< std::map<unsigned int, unsigned int>::iterator >::iterator
			iter2_end = iterators.end();
		...
		if (_single_trace_mode)
		{
			while ( (!should_stop()) && (!_single_trace_mode_event.wait(100)) );
			_single_trace_mode_event.reset();
			...
			for (; iter != iter_end; ++iter)
				_tile_cache->remove(*iter);
		}
		
		std::map<unsigned int, TraceProcessor*>::iterator search_result
			= _trace_processors.find(trace_processor_id);
...
/*	void
	...
	{
		const std::vector<Tile::Id>& needed_tile_ids
			= filtered_trace.needed_tile_ids();
		
		/* Check if all needed tiles are available. */
		std::vector<unsigned int>::const_iterator iter
			= needed_tile_ids.begin();
		std::vector<unsigned int>::const_iterator iter_end
			= needed_tile_ids.end();
		std::map<unsigned int, unsigned int>::iterator search_result;
		std::map<unsigned int, unsigned int>::iterator not_found_position
			= _locked_tiles.end();
