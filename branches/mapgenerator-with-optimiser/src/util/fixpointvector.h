...
namespace mapgeneration_util
{
	...
	class FixpointVector : protected std::vector< std::pair<bool, T_ElemType> >
	{
		...
		public:
			...
			/**
			 * \brief Reserves a capacity of at least num.
			 * Just calls the corresponding method in std::vector.
			 * @param num the resulting capacity (at least)
			 */			 
			inline void
	...
	template<typename T_ElemType>
	inline const std::vector<typename FixpointVector<T_ElemType>::size_type>&
	FixpointVector<T_ElemType>::free_positions() const
	{
	...
	template <typename T_Elem>
	void
	FixpointVector<T_Elem>::reserve(FixpointVector::size_type num)
	{
		std::vector< std::pair<bool, T_Elem> >::reserve(num);
	}
