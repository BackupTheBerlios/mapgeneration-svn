...
namespace mapgeneration_util
{
	...
	class GeoCoordinate
	{
		...
		public:
			...
			virtual
			~GeoCoordinate() {} // WE NEED IT. TRY TO ERASE THIS LINE *hehe*
			
			
			/**
			 * @brief Defines the default calculation methode for the
			 * bearing.
			 *
			 ...
			 * @param at_point the proportional value (used in
			 * bearing_on_great_circle)
			 * 
			 * @return the a value for bearing
			 */
			inline double
			bearing(const GeoCoordinate& geo_coordinate,
			 ...
			 * from this to the given GeoCoordinate.
			 *
			 * This method is input invariant! In spite of the method
			 * bearing_appromimated, which is little faster (one cosine less).
			 * 
			 * @param geo_coordinate the given GeoCoordinate
			 * @param output_representation a switch to control the
			 * representation of the return value. Default is radian. You can
			 ...
			 */
			double
			bearing_approximated2(const GeoCoordinate& geo_coordinate,
				const Representation output_representation = _RADIAN) const;
			...
			/**
			 * @brief Defines the default calculation methode for the
			 * computation of a GeoCoordinate given the polar coordinates.
			 *
			 ...
			 * is meter.
			 * 
			 * @return the GeoCoodinate
			 */
			inline GeoCoordinate
			compute_geo_coordinate(double bearing, double distance,
			...
			/**
			 * @brief Defines the default calculation method for the distance.
			 *
			 ...
			 */
			inline double
			distance(const GeoCoordinate& geo_coordinate,
			 ...
			 *
			 * This method is input invariant! In spite of the method
			 * distance_appromimated, which is little faster (one cosine less).
			 * 
			 * @param geo_coordinate the given GeoCoordinate
			 * 
			 * @return the approximated distance in the chosen representation.
			 */
			double
			distance_approximated2(const GeoCoordinate& geo_coordinate,
				const Representation output_representation = _METER) const;
			...
			inline bool
			has_perpendicular_on_segment(const GeoCoordinate& start_gc,
				const GeoCoordinate& end_gc) const;
			
			
			bool
			has_perpendicular_on_segment_approximated(
				const GeoCoordinate& start_gc, const GeoCoordinate& end_gc) const;
			
			
			bool
			has_perpendicular_on_segment_on_great_circle(
				const GeoCoordinate& start_gc, const GeoCoordinate& end_gc) const;
			
			
			/**
			 * @brief Defines the default calculation method for the
			 * interpolation of a GeoCoordinate.
			 *
			 ...
			 */
			static inline GeoCoordinate
			interpolate(const GeoCoordinate& gc_1, const GeoCoordinate& gc_2,
	...
	inline double
	GeoCoordinate::bearing(const GeoCoordinate& geo_coordinate,
		const Representation output_representation, const double at_point) const
	{
		return bearing_approximated(geo_coordinate, output_representation);
	}
	...
	GeoCoordinate::bearing_on_orthodrom(const GeoCoordinate& geo_coordinate,
		const Representation output_representation, const double at_point) const
	{
		return bearing_on_great_circle(geo_coordinate, output_representation,
			at_point);
	}
	
	
	inline GeoCoordinate
	GeoCoordinate::compute_geo_coordinate(double bearing, double distance,
		const Representation bearing_representation,
		const Representation distance_representation) const
	{
		return compute_geo_coordinate_approximated(bearing, distance,
			bearing_representation, distance_representation);
	}
	...
	inline double
	GeoCoordinate::distance(const GeoCoordinate& geo_coordinate,
		const Representation output_representation) const
	{
		return distance_approximated2(geo_coordinate, output_representation);
	}
	...
	inline bool
	GeoCoordinate::has_perpendicular_on_segment(const GeoCoordinate& start_gc,
		const GeoCoordinate& end_gc) const
	{
		return has_perpendicular_on_segment_approximated(start_gc, end_gc);
	}
	...
	GeoCoordinate::interpolate(const GeoCoordinate& gc_1,
		const GeoCoordinate& gc_2, const double weight_on_first)
	{
//		std::cout << "GeoCoordinate::interpolate: weight_on_first="
//			<< weight_on_first << std::endl;
//		std::cout << gc_1 << std::endl << gc_2 << std::endl;
//		std::cout << interpolate_approximated(gc_1, gc_2, weight_on_first) << std::endl;
		
		return interpolate_approximated(gc_1, gc_2, weight_on_first);
	}
