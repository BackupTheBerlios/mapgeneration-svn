...
//		std::cout << "after_date_time_parse; ";
		...
		const double weight_on_first)
	{
		...
		GPSPoint interpolated_gps_point(
			GeoCoordinate::interpolate(gpsp_1, gpsp_2, weight_on_first));
		
		interpolated_gps_point.set_direction(Direction::interpolate(
				gpsp_1.get_direction(), gpsp_2.get_direction(), weight_on_first)
		);
		
		interpolated_gps_point.set_time(gpsp_1.get_time() * weight_on_first + 
			gpsp_2.get_time() * (1.0 - weight_on_first));
