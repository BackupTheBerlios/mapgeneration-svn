...
//			Tile::const_iterator node_iter_end = current_tile.write().end();
			...
			FixpointVector<Node>::const_iterator nodes_iter = nodes.begin();
			FixpointVector<Node>::const_iterator nodes_iter_end = nodes.end();
			for (; nodes_iter != nodes_iter_end; ++nodes_iter)
			{
				if (nodes_iter->first)
				{
					Node::D_Edges::const_iterator edges_iter = 
						nodes_iter->second.edges().begin();
					Node::D_Edges::const_iterator edges_iter_end =
						nodes_iter->second.edges().end();
					for (; edges_iter != edges_iter_end; ++edges_iter)
					{
						Tile::Id nnorthing;
						Tile::Id neasting;
						Tile::split_tile_id(Node::tile_id(edges_iter->get_next_node_id()),
							nnorthing, neasting);
						...
						{
							TileCache::Pointer op = _tile_cache->get(
								Node::tile_id(edges_iter->get_next_node_id()));
							if (op != 0)
							{
								GeoCoordinate nnode = op->node(
									edges_iter->get_next_node_id());
								MapGenerationDraw::arrow(_gps_draw,
									nodes_iter->second, nnode);
							}
						}
					}
				}
