...
//		const double PI = 3.141592;
		...
		int int_radius = static_cast<int>(floor(radius * _zoom_factor));
		if (int_radius > 0)
			_dc->DrawCircle(x, y, int_radius);
		
	}
	
	
	void
	GPSDraw::cross(double latitude, double longitude, double length)
//	void
...
/*	void
	GPSDraw::text(double latitude, double longitude, const wxString& text)
	{
		...
		int font_size = (int)(2.0 * _zoom_factor);
	...
	GPSDraw::text(double latitude, double longitude, const wxString& text)
	{
		...
		int font_size = (int)(30.0 * _zoom_factor);
